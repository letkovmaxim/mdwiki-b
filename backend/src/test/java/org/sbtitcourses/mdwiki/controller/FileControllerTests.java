...
class FileControllerTests {
    ...
    void verifyHttpRequestMappingAndDeserialization() throws Exception {
        ...
        when(imageStorageService.storeImage(file, 1)).thenReturn(storedFile);

        mockMvc.perform(multipart("/spaces/1/upload/image").file(file))
                .andExpect(status().isOk());

        verify(imageStorageService).storeImage(file, 1);
    }
    ...
    @Test
    void verifyResultSerialization() {

    }

    @Test
    void verifyErrorHandling() throws Exception {
        when(imageStorageService.loadImage("x"))
                .thenThrow(ElementNotFoundException.class);
        when(imageStorageService.loadImage("y"))
                .thenThrow(AccessDeniedException.class);

        mockMvc.perform(get("/download/image/{GUID}", "x"))
                .andExpect(status().isNotFound());
        mockMvc.perform(get("/download/image/{GUID}", "y"))
                .andExpect(status().isForbidden());

        verify(imageStorageService).loadImage("x");
        verify(imageStorageService).loadImage("y");
    }
}